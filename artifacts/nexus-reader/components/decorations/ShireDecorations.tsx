...
export function ShireBackground() {
  ...
  return (
    <Svg width={width} height={height} style={StyleSheet.absoluteFill} pointerEvents="none">
      <Rect width={width} height={height} fill="#2d4128" />
      <Path d={`M 0 ${height * 0.7} C ${width * 0.18} ${height * 0.52}, ${width * 0.32} ${height * 0.82}, ${width * 0.52} ${height * 0.63} S ${width * 0.84} ${height * 0.86}, ${width} ${height * 0.68} L ${width} ${height} L 0 ${height} Z`} fill="#49633e" opacity={0.9} />
      {Array.from({ length: 7 }).map((_, i) => (
        <Path key={i} d={`M ${i * 85 + 20} ${40 + (i % 3) * 24} C ${i * 85 + 40} ${20 + (i % 2) * 10}, ${i * 85 + 56} ${60 + (i % 2) * 14}, ${i * 85 + 76} ${40 + (i % 3) * 20}`} stroke="#7f926f" strokeWidth={2} fill="none" opacity={0.2} />
      ))}
    </Svg>
  );
}
...
export function HobbitDoor() {
  return (
    <Svg width={60} height={84} pointerEvents="none">
      <Circle cx={30} cy={42} r={28} fill="#5ea84b" stroke="#22371c" strokeWidth={3} />
      <Rect x={26} y={33} width={8} height={20} rx={4} fill="#22371c" />
...
export function ElvenDeer() {
  return (
    <Svg width={120} height={86} pointerEvents="none">
      ...
      <Path d="M 72 40 Q 78 46 84 40" stroke="#22371c" strokeWidth={2} fill="none" strokeLinecap="round" />
    </Svg>
  );
}
